    ...
    def _recv_loop(self) -> None:
        ...
        while True:
            try:
                ...
                #A process broke, skip processing the receive
                if n_p not in self._processes:
                    continue
            except queue.Empty:
                break
...
class AudioUnpacker(_mp_ctx.Process):
    ...
    def run(self) -> None:
        ...
        while True:
            try:
                ...
                print(f"Audio Unpacker exception! Mode: {mode} DataLength: {len(data)}")
                pipe.send(exc)
                raise exc
                return
    ...
    def unpack_audio_packet(self, data: bytes, mode: str, decode: bool) -> Union[RTCPPacket, AudioFrame]:
        ...
        if decode and packet.audio != SILENT_FRAME:
            ...
            decodedAudioPacketData = self.decoders[packet.ssrc].decode(packet.audio)
            #print(f"Decoded Data Len: {len(decodedAudioPacketData)}")
            return AudioFrame(decodedAudioPacketData, packet, None)  # type: ignore
